...
public class TestLocalAssignmentsNewDesign extends AssignmentTestCase {
    ...
    public void testResourceAppearsInListAfterCreation() {
        ...
        myAssignmentManager.createAssignment(task, res1);
        myAssignmentManager.createAssignment(task, res2);
...
//    }
    ...
    private Set<HumanResource> extractResources(Task task) {
        ...
        for (LocalAssignment assignment: myAssignmentManager.getTaskAssignments(task)) {
            result.add(assignment.getResource());
            assertEquals("Unexpected task is owning resource assignment=" + assignment, task, assignment.getTask());
        }
        return result;
    }
}
